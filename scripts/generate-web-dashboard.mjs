...
function redactText(value, maxLength = 210) {
  let text = normalizeText(value)
    ...
    .replace(/\bSSDI\b/g, "benefit application")
    .replace(/\bJoan\b/gi, "recent service correction")
    .replace(/\bMichele\b/gi, "family relationship")
    ...
    .replace(/Ask primary care[^.]+context\./gi, "Ask qualified professionals whether recent health-marker flags need follow-up.")
    .replace(/\btherapy\b|\bpsychiatry\b|\bmedication(?:-management)?\b|\brefill\b|\bside effects\b|\bhemoglobin\b|\bhematocrit\b|\bBUN\b|\blipid-panel\b|\bclinician\b|\bprimary care\b|\battorney\b/gi, "professional-care")
...
function buildDashboard() {
  const config = JSON.parse(readRepo("dashboard-source/web-dashboard.json"));
